...
void Monster::Update()
{
	...
	if (onoff && !boom)
	{
		...
		case MON_BRING:
		{
			if (now - MonMoveTime >= 240)
			{
				...
				for (int i = 0; i < blockMgr->getIndex(); ++i)
				{
					...
					if (next_monX > blkx && next_monx < blkX && monY > blky && mony < blkY)
					{
						blocked = true;
						break;
					}
		...
		// ◆ 바닥 꺼짐 체크 – 블록 놓은 직후 예외 처리
		if (col_floor)
		{
			bool justPlacedBlockHere = 
				(
					state == MON_PLACE 
					&& !hasIcicle 
					&&targetX == -1 
			...
			{
				OutputDebugStringA("[INFO] 방금 설치한 블록 위 - 죽지 않음\n");
			}
			col_floor = false;
		}
	}
...
/*
기능 :		몬스터의 상태에 따른 스트라이프 출력(맵 이동 중)
매개변수 :	없음
반환 :		없음
특이사항 :	없음
*/
...
/*
기능 :		방향전환을 위한 충돌판정
매개변수 :	충돌을 판단하기위한 블록의 충돌 범위, 블록의 onoff
반환 :		충돌 판정 결과 - 블록과 충돌 시 1,2 / 블록 충돌 없을 시 0 
특이사항 :	없음
*/
...
{	
	...
	if (pos == RIGHT)
	{
		...
		// 몬스터의 약간 앞쪽 아래에 충돌점을 만들어 충돌했는지 판단(블록 상태가 off일 경우만)
		if (monX + 10 >= blockx &&
			monX + 10 <= blockX &&
			monY + 17 >= blocky &&
			monY + 17 <= blockY &&
...
/*
기능 :		바닥 파괴을 위한 충돌 판정
매개변수 :	충돌을 판단하기 위한 블록의 충돌 범위, 블록의 onoff
반환 :		충돌 판정 결과 - 블록과 충돌 시 1 / 블록 충돌 없을 시 0
특이사항 :	없음
*/
int Monster::Collision_floor(int blockx, int blockX, int blocky, int blockY, bool onoff)
{
	// 몬스터보다 약간 좁은 아랫 부분에 충돌 범위가 블록에 충돌했는지 판단(블록이 off일때만)
	if (((monx + 7 >= blockx && monx + 7 <= blockX) || (monX - 7 >= blockx && monX - 7 <= blockX)) 
		&&	(monY + 17 >= blocky && monY + 17 <= blockY) && !onoff)
	{
		// 충돌했으면 1
		return 1;											
	}
...
/*
기능 :		플레이어에게 공격당한 충돌 판정
매개변수 :	충돌을 판단하기 위한 플레이어의 충돌 범위, 플레이어의 방향
반환 :		충돌 판정 결과 - 충돌 시 1 / 충돌 없을 시 0
특이사항 :	없음
*/
int Monster::Collision_attacked(int chax, int chaX, int chay, int chaY, int pos)
{
	// 플레이어의 방향에 따라 플레이어 앞쪽에 충돌 범위 생성
	if (pos == LEFT)
	{
		chaX = chax;
		chax -= 45;
